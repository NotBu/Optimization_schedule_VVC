...
def approx_average_is_average(hand):
    ...
    """
    trung_binh_that = sum(hand) / len(hand)
    cach_nham_1 = (hand[0] + hand[-1]) / 2
    cach_nham_2 = hand[len(hand) // 2]
    return (cach_nham_1 == trung_binh_that) or (cach_nham_2 == trung_binh_that)
...
def average_even_is_average_odd(hand):
    ...
    odd = hand[::2]
    even = hand[1::2]
    tb_odd = sum(odd)/len(odd)
    tb_even = sum(even)/len(even)
    return tb_even == tb_odd
